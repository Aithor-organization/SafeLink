...
function buildMultimodalPrompt(data) {
  ...
  return `당신은 사이버 보안 전문가입니다. 다음 웹페이지를 분석하여 피싱, 스캠, 악성코드 위험도를 평가해주세요.
...
- HTML 구조: 로그인 폼, 결제 폼, 개인정보 입력 필드 존재 여부
- 폼 동작: 외부 서버로 데이터 전송, 숨겨진 필드 과다
- 스크립트: 난독화된 코드, 키로거 패턴, 리다이렉트 코드

### 2. 스크린샷 분석 (제공된 이미지 기반)
- 로고 위조: 유명 브랜드 로고 변형, 품질 저하된 이미지
- 공포/긴급 문구: "계정 정지", "즉시 조치 필요", "만료 임박" 등
- UI 위장: 은행/결제 서비스 위장, 팝업 위장 광고
- 신뢰성 배지: 가짜 보안 인증, 위조된 평점/리뷰
...
function buildMultimodalMessages(prompt, screenshot) {
  ...
  return [
    {
      role: 'system',
      content: `당신은 사이버 보안 전문가로, 웹사이트의 안전성을 멀티모달 분석(소스코드 + 시각적 분석)을 통해 평가합니다.
항상 JSON 형식으로 응답하며, 한국어로 분석 결과를 제공합니다.
주요 분석 영역:
1. 피싱 (Phishing): 정당한 서비스 위장
2. 스캠 (Scam): 사기성 유도
3. 악성코드 (Malware): 유해 스크립트
4. 의심 (Suspicious): 일반적 위험 징후`,
    },
    {
      role: 'user',
      content,
